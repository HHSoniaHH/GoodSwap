...
const AddOrganisations = ({getOrganByUser}) => {
    ...
    const handleSubmit = (e) => {
        ...
        formData.append('NomOrganisation', organ.NomOrganisation);

        formData.append('TelOrganisation', organ.TelOrganisation);
        formData.append('EmailOrganisation', organ.EmailOrganisation);
        formData.append('AdresseOrganisation', organ.AdresseOrganisation);
        formData.append('organisation', organ.image);


        axios.post(`http://localhost:4000/createOrgan/${id}`, formData,)
          .then(response => {
            ...
            toast.success(response.data.message, {
                ...
                window.localStorage.setItem('organisationId',response.data.data._id)
...
>
<CiCirclePlus  style={{fontWeight:'bold',fontSize:35}}/>   Ajouter mon organisation</button>
{showModal ? (
<>
...
<h3 className="text-3xl font-semibold">Ajouter mon organisation </h3>
<button
...
<input
  type="text"
  name="NomOrganisation" value={organ.NomOrganisation} onChange={handleChange}
...
<input
  type="number"
  name="TelOrganisation" value={Number(organ.TelOrganisation)} onChange={handleChange}
...
<div className="mb-2">
                    <input
                      type="email" name="EmailOrganisation" value={organ.EmailOrganisation} onChange={handleChange}
                  ...
                  <input
  type="text"
  name="AdresseOrganisation" value={organ.AdresseOrganisation} onChange={handleChange}
  class="bg-gray-50 outline-none border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-200 dark:border-gray-500 dark:placeholder-gray-400 dark:text-black"
  placeholder="AdresseOrganisation"
