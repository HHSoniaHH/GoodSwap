...
const addTicket = ({ GetAllEvent }) => {
  ...
  const handleSubmit = (e) => {
    ...
    // formData.append('titre', organ.titre);

    // formData.append('nbrParticipant', organ.nbrParticipant);
    // formData.append('EmailOrganisation', organ.EmailOrganisation);
    // formData.append('location', organ.location);
    // formData.append('organisation', organ.image);

    axios
      .post(`http://localhost:4000/createEvent/${organisationId}`, events)
      .then((response) => {
  ...
  return (
    <>
      ...
      >
        <CiCirclePlus style={{ fontWeight: "bold", fontSize: 35 }} /> Ajouter un
        evenement
      </button>
      {showModal ? (
        <>
                  ...
                  <form class="w-full mt-4 " onSubmit={handleSubmit}>
                    ...
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-2 mb-2 ">
                      <input
                        type="number"
                        name="nbrParticipant"
                        value={Number(events.nbrParticipant)}
                        onChange={handleChange}
                        class="bg-gray-50  outline-none border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-200 dark:border-gray-200 dark:placeholder-gray-400 dark:text-black"
                        placeholder="nbrParticipant"
                      />
                      <input
                        type="number"
                        name="nbrTelespectateur"
                        value={Number(events.nbrTelespectateur)}
                        onChange={handleChange}
                        class="bg-gray-50  outline-none border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-200 dark:border-gray-200 dark:placeholder-gray-400 dark:text-black"
                        placeholder="nbrTelespectateur"
                      />
                    </div>
...
/>
                    ...
                    <div className=" mb-2">
                      <textarea
                        ...
                        placeholder="Description d'evenement"
                      ></textarea>
                    </div>
