...
const filteredUsers = users.filter(i => i.UserType === 'Organisateur');
const handleUpdateUser = () => {
  axios.put(`http://localhost:4000/user/${editUser._id}`, editUser,{  headers: {
      Authorization: `JWT ${token}`,
    },})
    .then(response => {
...
const deleteUser = async (id, name) => {
  ...
  if (
    window.confirm(`Vous voulez vraiment supprimer ${name} !`)
...
const updateUser = async (id, name) => {
  ...
  if (
    window.confirm(`Est ce que vous voulez vraiment modifier ${name}!!!!`)
...
getAllUser(); 
  ...
  return (
   < > <tr>
                ...
                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                  <i className="fas fa-circle text-green-500 text-sm ">{user.status.toString()}</i> 
                </td>
                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
             ...
             {user.UserType}
                ...
                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                  ...
                  <button
          href="#"
          class="font-medium bg-white p-1 w-16 rounded  text-light-600 dark:text-blue-500 hover:text-blue-300"
              ...
              <div class="px-12 py-8 w-full">
                ...
                <form class="w-full mt-4 ">
                 <div className="grid grid-cols-1 gap-3 md:grid-cols-2 mb-6 ">
                  ...
                  <div className="mb-6">
                    <input
                      type="email" name="email" value={editUser.email||user.email} onChange={handleInputChange}
